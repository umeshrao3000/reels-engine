...
- [ ] 🔴 A new customer can sign up without staff intervention — *MR-5, superseded by MR-3.1*. **MR-3.1 evidence:** see the Authentication section below.
- [ ] 🔴 A new customer can connect their own real Instagram account via OAuth — *MR-5, superseded in part by MR-3.2*. **MR-3.2 evidence:** `app/api/customer/instagram/connect` + the shared callback (`app/api/admin/instagram/callback/route.ts`, dispatched by state cookie) implement the full code path — CSRF-protected, organization-scoped, with a cross-organization reconnect-conflict guard — verified in `test/api/customer-ownership.test.ts` and `lib/modules/meta/__tests__/oauth-state.test.ts`. Not checked — real Meta OAuth against a live app can't run in CI (`e2e/customer-workspace.spec.ts` seeds the connected account directly); end-to-end verification against a real Meta test account remains *MR-9*.
- [ ] 🔴 A new customer can create a campaign and keywords and see it go live — *MR-5, superseded in part by MR-3.2*. **MR-3.2 evidence:** `/dashboard/campaigns` (create/edit/delete, keyword management) is complete and organization-scoped, reusing the exact same `lib/modules/keywords/keyword-service.ts` and matching pipeline the admin surface already used — verified end to end via a real browser in `e2e/customer-workspace.spec.ts` (create campaign → manage keywords). Not checked — "go live" against real Meta production traffic is unverified until a real connected account exists (see the item above).
...
- [ ] 🔴 A distinct internal super-admin role exists, separate from workspace-owner accounts — *MR-4*. The `Organization` (workspace-owner) concept now exists as of MR-3.2 (`docs/ORGANIZATIONS.md`), but a distinct internal super-admin role does not — `/ops` remains the single shared `ADMIN_PASSCODE`, unaffected by and unaware of organizations, per MR-3.2's explicit scope boundary.
...
- [ ] 🔴 `Workspace`/`User` models exist and every automation-related model is workspace-scoped — *MR-4, superseded in part by MR-3.2*. **MR-3.2 evidence:** `Organization` model exists (`User.organization`, `@unique` on `ownerUserId` — exactly one per user), `docs/ORGANIZATIONS.md`. `SocialAccount` and `ConversionLog` carry `organizationId`; `Campaign`/`Keyword` are scoped by join through the existing chain rather than a redundant column (see that doc for why). `Lead` is deliberately not organization-scoped — an existing global-identity invariant this milestone was not authorized to redesign, also documented there. Not checked — this is the customer-facing (`/dashboard`) surface only; `/ops` remains fully unscoped by design (see the Authentication section above), which is the correct state until MR-4's internal super-admin role exists, not a gap in this item.
- [ ] 🔴 Every `/ops`-equivalent query is provably scoped to the authenticated workspace (no unscoped `findMany` remains) — *MR-4*. Not applicable to `/ops` itself yet (unaffected by design, see above) — `/dashboard`'s equivalent queries are scoped: verified by `test/api/customer-ownership.test.ts` (every `/api/customer/**` route) and `e2e/customer-workspace.spec.ts` (the actual UI).
- [ ] 🔴 Cross-tenant data-isolation test suite passes (two workspaces, zero leakage, every resource type) — *MR-4, superseded in part by MR-3.2*. **MR-3.2 evidence:** `lib/modules/organizations/__tests__/ownership.test.ts` (unit level) and `test/api/customer-ownership.test.ts` (real HTTP: two real organizations, campaigns/keywords/Instagram-disconnect routes, every cross-org attempt asserted 404 — not 403 — and asserted to leave the other organization's data unmodified) both pass. Not checked — scoped to the resource types this milestone introduced ownership for (`SocialAccount`, `Campaign`, `Keyword`, `ConversionLog`); does not cover `/ops` resources, which remain intentionally unscoped.
- [ ] 🟡 Manual cross-tenant access attempt (URL manipulation) confirmed blocked — *MR-9*. **MR-3.2 evidence:** `e2e/customer-workspace.spec.ts` confirms this in a real browser for `/dashboard/campaigns` and `/dashboard/instagram` (a second customer's workspace shows neither the first customer's campaign nor their connected account). Not checked — MR-9's full manual pass (direct URL manipulation to another organization's resource ids, browser DevTools, etc.) is a separate, broader exercise.
...
- [ ] 🟡 OAuth CSRF state comparison uses constant-time comparison throughout (currently one plain `!==` gap at `lib/modules/meta/oauth-state.ts:33`, in the pre-existing admin `isOAuthStateValid` — left untouched, unfixed, still tracked here) — *MR-2 or MR-4, whichever touches this file first*. **MR-3.2 note:** this milestone added `createCustomerOAuthState`/`readCustomerOAuthState` to the same file for the customer Instagram-connect flow, written with a fully constant-time comparison from the start (and its own domain-separation tag, so a signed admin state can never be replayed as a valid customer state despite sharing `OAUTH_STATE_SECRET` — caught by a red test during development, `lib/modules/meta/__tests__/oauth-state.test.ts`). The admin function's pre-existing gap was deliberately left as-is, not silently fixed, since hardening it was out of this milestone's scope.
